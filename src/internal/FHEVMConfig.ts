...
export function generateFHEVMConfigDotSol(fhevmEnv: FhevmEnvironment, addresses: FHEVMConfig): string {
  const dstPath = fhevmEnv.paths.cacheFHEVMConfigSol;
  ...
  const origPath = path.join(fhevmEnv.paths.fhevmSolidityConfig, "FHEVMConfig.sol");
  ...
  assert(origContent.indexOf("../lib/FHE.sol") >= 0);
  assert(origContent.indexOf("../lib/Impl.sol") >= 0);
  assert(origContent.indexOf("ACLAddress: 0xFee8407e2f5e3Ee68ad77cAE98c434e637f516e5") >= 0);
  assert(origContent.indexOf("FHEVMExecutorAddress: 0x687408aB54661ba0b4aeF3a44156c616c6955E07") >= 0);
  assert(origContent.indexOf("KMSVerifierAddress: 0x9D6891A6240D6130c54ae243d8005063D05fE14b") >= 0);
  assert(origContent.indexOf("InputVerifierAddress: 0x3a2DA6f1daE9eF988B48d9CF27523FA31a8eBE50") >= 0);
