...
// function __checkIsHardhatSignerAddress(hhSigners: HardhatEthersSigner[], address: string) {
//   if (!__isHardhatSignerAddress(hhSigners, address)) {
//     throw new Error(
...
function __getStringConstant(name: keyof typeof constants, defaultValue?: string, dotenvRelPath?: string): string {
  if (defaultValue) {
    if (constants[name] !== defaultValue) {
      throw new Error("ERROR !!! __getStringConstant");
    }
  }
...
export function getGatewayInputVerificationAddress(): string {
  try {
    ...
    if (!EthersT.isAddress(addr)) {
      throw new Error(`Invalid InputVerification contract address: ${addr} (Input Verifying contract source address)`);
    }
    return addr;
  } catch {
    // Default
...
export async function getCoprocessorSigners(hre: HardhatRuntimeEnvironment): Promise<EthersT.Signer[]> {
  /*
  ...
  try {
    ...
    const signer = new EthersT.Wallet(coprocessorSignerKey).connect(hre.ethers.provider);
    return [signer];
  }
}
...
*/
export async function getKMSSigners(hre: HardhatRuntimeEnvironment): Promise<EthersT.Signer[]> {
  try {
    const hhSigners = await hre.ethers.getSigners();
    ...
    const signer = new EthersT.Wallet(kmsSignerKey).connect(hre.ethers.provider);
    return [signer];
  }
}

function envGetList(envVarNamePrefix: string): string[] {
  envVarNamePrefix = ensureSuffix(envVarNamePrefix, "_");
...
function _resolveCoreContractsArtifactsPath(artifactsRelPath: string) {
  let modulePath = path.resolve(artifactsRelPath);
  if (!fs.existsSync(modulePath)) {
    modulePath = path.join(constants.FHEVM_CORE_CONTRACTS_PACKAGE_NAME, artifactsRelPath);
  }
  return modulePath;
}

// @zama-fhe/oracle-solidity/artifacts/contracts/...
function _resolveZamaFheOracleSolidityArtifactsPath(artifactsRelPath: string) {
  let modulePath = path.resolve(artifactsRelPath);
  if (!fs.existsSync(modulePath)) {
    modulePath = path.join(constants.ZAMA_FHE_ORACLE_SOLIDITY_PACKAGE_NAME, artifactsRelPath);
  }
  return modulePath;
}
...
// async function getACL(runner?: EthersT.ContractRunner | null | undefined): Promise<EthersT.Contract> {
//   const artifact = await getCoprocessorArtifact("ACL");
//   const address = getACLAddress();
//   return new EthersT.Contract(address, artifact.abi, runner);
// }
...
// async function getDecryptionOracle(
//   runner?: EthersT.ContractRunner | null | undefined,
// ): Promise<EthersT.Contract> {
//   const artifact = await getGatewayArtifact("DecryptionOracle");
//   const address = getDecryptionOracleAddress();
//   return new EthersT.Contract(address, artifact.abi, runner);
// }
...
// async function getFHEVMExecutor(runner?: EthersT.ContractRunner | null | undefined): Promise<EthersT.Contract> {
//   const artifact = await getCoprocessorArtifact("FHEVMExecutor");
//   const address = getFHEVMExecutorAddress();
//   return new EthersT.Contract(address, artifact.abi, runner);
// }
...
// async function getInputVerifier(runner?: EthersT.ContractRunner | null | undefined): Promise<EthersT.Contract> {
//   const artifact = await getCoprocessorArtifact("InputVerifier");
//   const address = getInputVerifierAddress();
//   return new EthersT.Contract(address, artifact.abi, runner);
// }
...
// async function getKMSVerifier(runner?: EthersT.ContractRunner | null | undefined): Promise<EthersT.Contract> {
//   const artifact = await getCoprocessorArtifact("KMSVerifier");
//   const address = getKMSVerifierAddress();
//   return new EthersT.Contract(address, artifact.abi, runner);
// }
...
// async function getFHEGasLimit(runner?: EthersT.ContractRunner | null | undefined): Promise<EthersT.Contract> {
//   const artifact = await getCoprocessorArtifact("FHEGasLimit");
//   const address = getFHEGasLimitAddress();
//   return new EthersT.Contract(address, artifact.abi, runner);
// }
