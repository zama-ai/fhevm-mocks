...
import { getMaxBigInt, isUInt } from "../utils/math";
import { FheType, getFheTypeBitLength } from "./FheType";
import { ALL_FHE_TYPES, checkFheType } from "./FheType";
...
export function checkFhevmType(fhevmType: unknown): asserts fhevmType is FhevmType {
  if (!isFhevmType(fhevmType)) {
    throw new HardhatFhevmError(`Invalid FhevmType ${fhevmType}`);
  }
}
...
export function FheTypeToFhevmType(fheType: FheType): FhevmType {
  ...
  if (fheType >= allFhevmTypeInfos.length) {
    throw new HardhatFhevmError(`Cannot convert FheType ${fheType} to FhevmType`);
  }
...
export function FhevmTypeToFheType(fhevmType: FhevmType): FheType {
  ...
  if (fhevmType >= ALL_FHE_TYPES.length) {
    throw new HardhatFhevmError(`Cannot convert FhevmType: ${fhevmType} to FheType`);
  }
