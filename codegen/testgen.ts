...
function generateIntroTestCode(shards: OverloadShard[], idxSplit: number): string {
  ...
  shards.forEach((os) => {
    intro.push(`
  import type { FHEVMTestSuite${os.shardNumber} } from '../../../typechain-types/contracts/operators/FHEVMTestSuite${os.shardNumber}';
  `);
  });
  ...
  shards.forEach((os) => {
    os.overloads.forEach((o) => {
      ...
      tests.forEach((t) => {
        ...
        const fhevmTypeArg = o.returnType.type === 1 ? `FhevmType.euint${o.returnType.bits}, ` : "";

        res.push(`
                it('${testName} test ${testIndex} (${testArgs})', async function () {
                    const input = hre.fhevm.createEncryptedInput(this.contract${os.shardNumber}Address, this.signers.alice.address);
                    ${inputsAdd}
                    const encryptedAmount = await input.encrypt();
                    const tx = await this.contract${os.shardNumber}.${methodName}(${testArgsEncrypted}, encryptedAmount.inputProof);
                    await tx.wait();
                    const res = await hre.fhevm.debugger.decrypt${o.returnType.type === 1 ? "Euint" : "Ebool"}(${fhevmTypeArg} await this.contract${os.shardNumber}.res${o.returnType.type === 1 ? `Euint${o.returnType.bits}` : "Ebool"}());
...
export function generateSolidityUnitTestContracts(os: OverloadShard): string {
  ...
  res.push(`
        ...
        contract FHEVMTestSuite${os.shardNumber} is SepoliaFHEVMConfig {
