...
function generateAllFiles() {
  ...
  mkdirSync("contracts/operators", { recursive: true });
  overloadShards.forEach((os) => {
    writeFileSync(`contracts/operators/FHEVMTestSuite${os.shardNumber}.sol`, generateSolidityUnitTestContracts(os));
  });

  const tsSplits: string[] = generateTypeScriptTestCode(overloadShards, numberOfTestSplits);
  tsSplits.forEach((split, splitIdx) =>
    writeFileSync(`test/hardhat-mock-engine/operators/fhevmOperations${splitIdx + 1}.ts`, split),
  );
}
