...
//
// The v12 TypeScript face: every constant v12 has a field for, in declaration order — the shared set
// plus what the JSON scopes to v12 with `generations`. The JSON is THE source of truth for every value the
// cleartext stack's languages must agree on: it records the keccak FORMULA behind each derived value rather
// than only the hex, and v12's `test/cleartext-config-mirror.test.ts` checks its copy of this module
// against it — name for name, in declaration order, value for value, bigint-vs-number literal shape included.
// v12's `pkg/src/cleartext/shared/LibFhevmCleartextConfig.sol` is the same JSON's Solidity face.
//
// `fhevm-npm sync vendored` copies this file into v12's pkg/ts/ as `cleartext-config.ts` — a published
