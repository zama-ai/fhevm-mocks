...
library LibFhevmCleartextConfig {
    ...
    /// v14 only. First half of `mpcIdentity`: node i is `<prefix><i + 1><infix><i + 1>`.
    string internal constant CLEARTEXT_KMS_NODE_MPC_IDENTITY_PREFIX = "kms-core-";

    /// v14 only. Second half of `mpcIdentity`, naming the cluster as cleartext.
    string internal constant CLEARTEXT_KMS_NODE_MPC_IDENTITY_INFIX = "-cleartext-core-";

    /// v14 only. Prefix inside the storage bucket; node i gets `<prefix><i + 1>`.
    string internal constant CLEARTEXT_KMS_NODE_PUBLIC_STORAGE_PREFIX = "cleartext-";

    /// v14 only. The KMS `softwareVersion` a context is created under. No build exists.
    string internal constant CLEARTEXT_KMS_SOFTWARE_VERSION = "0.0.0-cleartext";

    /// v14 only. `KmsNodeParams.caCert` as hex. Empty: there is no MPC peer to verify.
