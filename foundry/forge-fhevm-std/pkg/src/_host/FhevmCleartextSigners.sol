 ...
 *      rather than in a private copy per library. `FhevmCleartextEncrypt` signs as coprocessors,
 *      `FhevmCleartextDecrypt` and `FhevmCleartextDecryptPublic` as KMS nodes.
 *
 ...
 *      `FhevmCleartextConfig`. Those are independently generated facts, so a derived address is always
...
library FhevmCleartextSigners {
    ...
    function coprocessorKey(uint256 index) internal pure returns (uint256 privateKey) {
        ...
        privateKey = fvm.deriveKey(
            ...
            FhevmCleartextConfig.CLEARTEXT_COPROCESSORS_MNEMONIC_INDEX + uint32(index)
        );
    ...
    function kmsNodeKey(uint256 index) internal pure returns (uint256 privateKey) {
        ...
        privateKey = fvm.deriveKey(
            ...
            FhevmCleartextConfig.CLEARTEXT_KMS_NODES_MNEMONIC_INDEX + uint32(index)
        );
     ...
     *      order can use all of it — `FhevmCleartextDecryptPublic` does exactly that.
     *
