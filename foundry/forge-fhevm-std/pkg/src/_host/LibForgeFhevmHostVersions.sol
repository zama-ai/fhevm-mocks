...
import {LocalHostVersions} from "./_internal/LocalHostVersions.sol";
 ...
 *      down with it.
 */
library LibForgeFhevmHostVersions {
    ...
    IForgeVm private constant vm = IForgeVm(FORGE_VM_ADDRESS);

    // -- The floor: fhevm v0.13.0 -------------------------------------------
    //
    // The tags of `zama-ai/fhevm` at the start of this line. The ceilings below are GENERATED from what
    // this package vendors, so the upper end follows the release; only the lower end is written down.
    ...
    string internal constant HCU_LIMIT_FLOOR = "HCULimit v0.3.0";
    string internal constant KMS_GENERATION_FLOOR = "KMSGeneration v0.1.0";

    // NO PREVIOUS LINE, DELIBERATELY. The next generation carries bounds for the line below it, because
    // chains it can fork are still running that line and it brings them forward rather than refusing.
    // Here there is no such chain -- nothing in production runs the line below this one -- so a
    // `Previous` classification would be a branch no test could reach and no user could trigger.
    // `classify` therefore answers `Current` or `Unknown`, and `Unknown` is refused by name.
    ...
    function classify(HostVersions memory versions) internal pure returns (FhevmGeneration) {
        // `Previous` is never returned on this line -- see the floors above.
        return _isCurrentLine(versions) ? FhevmGeneration.Current : FhevmGeneration.Unknown;
    }
    ...
    /**
     * @notice The seven `getVersion()` strings of the stack at these addresses.
     * @dev A CONTRACT THAT CANNOT ANSWER READS AS ABSENT, not as an error: an address with no code, or
     *      one answering something else, yields `""`, and `classify` treats an empty string as a reading
     *      rather than a failure. That is what lets a chain missing `KMSGeneration` -- five of the eight
     *      in the table have none -- be classified instead of reverting halfway through.
     */
    function read(
        address acl,
    ...
    ) internal view returns (HostVersions memory versions) {
        versions.acl = versionOf(acl);
        versions.fhevmExecutor = versionOf(fhevmExecutor);
        versions.kmsVerifier = versionOf(kmsVerifier);
        versions.inputVerifier = versionOf(inputVerifier);
        versions.hcuLimit = versionOf(hcuLimit);
        versions.protocolConfig = versionOf(protocolConfig);
        versions.kmsGeneration = versionOf(kmsGeneration);
    }

    /// @notice `getVersion()` on `target`, or `""` where there is nothing to ask.
    function versionOf(address target) internal view returns (string memory) {
