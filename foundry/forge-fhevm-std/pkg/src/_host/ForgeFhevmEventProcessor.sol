...
/**
 * @title ForgeFhevmEventProcessorDB
 ...
 *         `ForgeFhevmEventProcessor._applyRand`). The mark is a LABEL, not a guard: it is what
 ...
 */
contract ForgeFhevmEventProcessorDB is ICleartextDB {
    enum Status {
        Unset,
        Known,
        NonReplayable
...
/**
 * @title ForgeFhevmEventProcessor
 * @notice Rebuilds every handle's cleartext from `FHEVMExecutor`'s EVENTS alone. Deployed as a PAIR
 *         with its own `ForgeFhevmEventProcessorDB`, which it creates in its constructor:
 *
 ...
 */
contract ForgeFhevmEventProcessor is CleartextArithmeticBase {
    ...
    mapping(address executor => ForgeFhevmEventProcessorDB store) private _dbOf;
    ...
    /// @notice Something asked for the selected executor's store, and no executor has been added.
    /// @dev A processor is legitimately empty until `addExecutor` is called — construction adds none —
    ///      so this is the state to name rather than a zero address to hand back.
    error CleartextEventNoExecutor();
    ...
    function addExecutor(address executor) public {
        ...
        ForgeFhevmEventProcessorDB store = new ForgeFhevmEventProcessorDB(address(this));
        _dbOf[executor] = store;
        // NOT persistent, on purpose: a store is the reconstruction of ONE chain's state and lives in the
        // execution context — in-memory chain or fork — that chain state lives in. Switch context and it is
        // gone with the chain; switch back and it is there. forge-fhevm-std creates one processor per
    ...
    /**
     * @notice Marks a contract that rebuilds cleartexts by replaying events.
     *
     * @dev Probed rather than assumed, exactly as `IS_CLEARTEXT` is: a decryption wants to drain any
     *      pending events before it reads, and the thing it reads from may be a plain store with
     *      nothing to drain. A marker keeps that from being a swallowed failed call.
     */
    bool public constant IS_FHE_EVENT_REPLAYER = true;
    ...
    function dbOf(address executor) public view returns (ForgeFhevmEventProcessorDB) {
        ForgeFhevmEventProcessorDB store = _dbOf[executor];
    ...
    function db() public view returns (ForgeFhevmEventProcessorDB) {
        if (_selected == address(0)) revert CleartextEventNoExecutor();
    ...
    // -----------------------------------------------------------------------
    // Unknown handles — see `ForgeFhevmEventProcessorDB.UnknownHandlePolicy`
    // -----------------------------------------------------------------------
    ...
    function revertOnUnknownHandles() public {
        db().setUnknownHandlePolicy(ForgeFhevmEventProcessorDB.UnknownHandlePolicy.Revert, 0, 0);
    }
    ...
    function useDeterministicUnknownHandles() public {
        db().setUnknownHandlePolicy(ForgeFhevmEventProcessorDB.UnknownHandlePolicy.Deterministic, 0, 0);
    }
    ...
    function useRandomUnknownHandles() public {
        db().setUnknownHandlePolicy(ForgeFhevmEventProcessorDB.UnknownHandlePolicy.Random, fvmRandom.randomUint(), 0);
    }
    ...
    function useFixedUnknownHandles(uint256 value) public {
        db().setUnknownHandlePolicy(ForgeFhevmEventProcessorDB.UnknownHandlePolicy.Fixed, 0, value);
    }

    /// @notice What the store knows about `handle`, without reverting. `plaintexts` is the read.
    function statusOf(bytes32 handle) public view returns (ForgeFhevmEventProcessorDB.Status) {
    ...
    function _apply(bytes32 topic, bytes memory data) private returns (bool) {
        ...
        if (topic == LibFhevmEvents.FheMulDiv.selector) return _applyMulDiv(data);
    ...
    /// @dev `fheMulDiv` is recorded with the OPERAND type, which is `factor1`'s: the divisor is always a
    ///      plaintext and `factor2` may be one (`scalarByte == 0x03`), so `factor1` is the one handle that
    ///      is always there to read the width off.
    function _applyMulDiv(bytes memory data) private returns (bool) {
        (bytes32 factor1, bytes32 factor2, bytes32 divisor, bytes1 scalarByte, bytes32 result) =
            abi.decode(data, (bytes32, bytes32, bytes32, bytes1, bytes32));

        recordMulDiv(result, factor1, factor2, divisor, scalarByte, LibFhevmHandle.typeOf(factor1));
        return true;
    }
