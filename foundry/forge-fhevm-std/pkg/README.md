...
Each line maps an import prefix to a folder. For example, `import "@fhevm/forge/TestFhevm.sol"`
resolves to `node_modules/@fhevm/forge/src/TestFhevm.sol`.
...
1. **Install the packages:**
   ...
   ```sh
   npm install @fhevm/forge @fhevm/solidity encrypted-types
   ```
...
**VS Code shows `File outside of allowed directories` on `@fhevm/forge/...` imports.** This happens when
`@fhevm/forge` is installed from a local `file:` path. `node_modules/@fhevm/forge` is then a symlink
