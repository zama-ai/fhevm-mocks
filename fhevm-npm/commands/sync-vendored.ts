...
export function syncVendoredCommand(options: {
  ...
  if (options.verbose) {
    console.log(
      options.digest === true
        ? '🔑 sync-vendored: computing each pinned digest from upstream'
        : options.check
          ? '🔎 sync-vendored: comparing every destination'
          : '📄 sync-vendored: writing every destination',
    );
  }

  // `--digest` is about the pinned trees only: the shared TypeScript has no upstream commit to download.
  if (options.digest === true) {
    const only = syncPinnedVendored(withProgress);
    return {
      command: 'sync-vendored --digest',
      checkedPackageKeys: [...only.destinations],
