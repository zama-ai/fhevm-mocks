...
export function parseCliOptions(argv: readonly string[]): CliOptions {
  ...
  let scaffoldOptions:
    | { readonly selector: string; readonly out: string; readonly registry?: string; readonly force: boolean }
    | undefined;
...
`,
  ...
  sync
    ...
    .option('--digest', 'download the pinned trees and print the digest each one implies, writing nothing', false)
  ...
  program
    ...
    .description('Copy a package out as a standalone project a user could install from a registry.')
    .requiredOption('-o, --out <path>', 'directory to write the project into')
    // Defaulted, because scaffolding exists to rehearse a user install against locally built packages,
    // and npmjs cannot serve those. Pass the flag to point somewhere else.
    .option(
      '--registry <url>',
      "install @fhevm packages from this registry; 'npmjs' means the public one",
      'http://127.0.0.1:4873',
    )
    .option('-f, --force', 'replace the output directory when it already has files in it', false)
    .action(
      (
        packageSelector: string,
        options: { readonly out: string; readonly registry?: string; readonly force: boolean },
      ) => {
        scaffoldOptions = {
