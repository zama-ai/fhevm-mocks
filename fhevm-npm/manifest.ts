...
const pinnedSourceSchema = z
  .object({
    ...
    // Optional so a new pin can be added before its copies exist; the checks report an absent digest
    // rather than the schema refusing the file, which would make bootstrapping a pin impossible.
    digest: z
      .string()
      .regex(/^sha256-[A-Za-z0-9+/]+={0,2}$/, 'must be a sha256- digest as `sync vendored` prints it')
      .optional(),
  })
  .strict();
