...
// Two lines, then a blank one. The second names the generation on purpose: `sync vendored` copies this
// file into the generation's pkg/ts under the stable name `cleartext-config.ts`, and an unscoped JSON
// gives every generation the same constants, so without this line two generations' copies would be
// byte-identical and a copy could not be traced back to the generation that owns it.
