...
export function renderPackageJson(
  ...
  // `scaffold` renders a mirror-only payload, which is never published but still has to name its links
  // by range for anyone installing it from a registry. Every publish path reaches this through
  // resolvePayload, which refuses a non-npm payload first, so the default keeps that guard where it is.
  { requireNpmDistribution = true }: { readonly requireNpmDistribution?: boolean } = {},
): RenderedPackageJson {
  if (requireNpmDistribution && !isNpmDistributedPayload(pkg))
    throw new Error(`publish render: ${pkg.key} is not an npm-distributed payload`);
