...
describe('FHEVM manual operations', function () {
  ...
  it('Select works returning if false', async function () {
    ...
    const tx = await contract.test_select(
      ...
      at(encryptedAmount.encryptedValues, 2),
      encryptedAmount.inputProof,
    );
    await tx.wait();
    const res = await fhevm.cleartextDb.readUint32({ euint32: (await contract.resEuint32()) as Hex });
  ...
  it('Select ebool', async function () {
    ...
    await tx.wait();
    const res = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
    expect(res).to.equal(false);
    const tx2 = await contract.test_select_ebool(false, false, true);
    await tx2.wait();
    const res2 = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
  ...
  it('Select works for eaddress returning if false', async function () {
    const encryptedAmount = await fhevm.client.encryptValues({
      values: [
        { type: 'bool', value: false },
        { type: 'address', value: '0x8ba1f109551bd432803012645ac136ddd64dba72' },
        { type: 'address', value: '0x8881f109551bd432803012645ac136ddd64dba72' },
      ],
      contractAddress: contractAddress,
      userAddress: signers.alice.address,
    });
    ...
    await tx.wait();
    const res = await fhevm.cleartextDb.readAddress({ eaddress: (await contract.resAdd()) as Hex });
  ...
  it('ebool eq ebool', async function () {
    ...
    await tx3.wait();
    const res3 = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
    expect(res3).to.equal(false);
    const tx4 = await contract.eqEbool(true, false);
    await tx4.wait();
    const res4 = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
  ...
  it('eaddress eq eaddress,eaddress false', async function () {
    const encryptedAmount = await fhevm.client.encryptValues({
      values: [
        { type: 'address', value: '0x8ba1f109551bd432803012645ac136ddd64dba72' },
        { type: 'address', value: '0x9ba1f109551bd432803012645ac136ddd64dba72' },
      ],
      contractAddress: contractAddress,
      userAddress: signers.alice.address,
    });
  ...
  it('eaddress eq scalar eaddress,address true', async function () {
    ...
    const encryptedAmount = await fhevm.helpers.encryptAddress({
      value: '0x8ba1f109551bd432803012645ac136ddd64dba72',
      contractAddress: contractAddress,
      userAddress: signers.alice.address,
    });
    const tx = await contract.test_eq_eaddress_address(encryptedAmount.externalEaddress, b, encryptedAmount.inputProof);
    await tx.wait();
    const res = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
    expect(res).to.equal(true);
  });
  ...
  it('eaddress eq scalar address,eaddress true', async function () {
    ...
    const tx = await contract.test_eq_address_eaddress(encryptedAmount.externalEaddress, b, encryptedAmount.inputProof);
    await tx.wait();
    const res = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
    expect(res).to.equal(true);
  });
  ...
  it('eaddress ne scalar eaddress,address false', async function () {
    ...
    const tx = await contract.test_ne_eaddress_address(encryptedAmount.externalEaddress, b, encryptedAmount.inputProof);
    await tx.wait();
    const res = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
    expect(res).to.equal(false);
  });
  ...
  it('eaddress ne scalar address,eaddress false', async function () {
    ...
    const tx = await contract.test_ne_address_eaddress(encryptedAmount.externalEaddress, b, encryptedAmount.inputProof);
    await tx.wait();
    const res = await fhevm.cleartextDb.readBool({ ebool: (await contract.resEbool()) as Hex });
    expect(res).to.equal(false);
  });
  ...
  it('ebool to euint8 casting works with true', async function () {
    ...
    await tx.wait();
    const res = await fhevm.cleartextDb.readUint8({ euint8: (await contract.resEuint8()) as Hex });
  ...
  it('ebool to euint16 casting works with true', async function () {
    ...
    await tx.wait();
    const res = await fhevm.cleartextDb.readUint16({ euint16: (await contract.resEuint16()) as Hex });
    expect(res).to.equal(1);
  });
  ...
  it('ebool to euint64 casting works with true', async function () {
    ...
    await tx.wait();
    const res = await fhevm.cleartextDb.readUint64({ euint64: (await contract.resEuint64()) as Hex });
  ...
  it('ebool to euint128 casting works with true', async function () {
    ...
    await tx.wait();
    const res = await fhevm.cleartextDb.readUint128({ euint128: (await contract.resEuint128()) as Hex });
    expect(res).to.equal(1n);
  });
  ...
  it('ebool to euint256 casting works with true', async function () {
    ...
    await tx.wait();
    const res = await fhevm.cleartextDb.readUint256({ euint256: (await contract.resEuint256()) as Hex });
    expect(res).to.equal(1n);
  });
