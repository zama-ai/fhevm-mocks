...
import { COPROCESSOR_EVENT_NAMES, parseCoprocessorEvents } from '#esm/internal/events.js';
import { FhevmCleartextContractsRepository, type FhevmContractWrapper } from '#esm/internal/contracts.js';

/** The deployed FHEVMExecutor wrapper, as `fhevm.revertedWithCustomErrorArgs` used to hand out. */
function executorOf(connection: { provider: unknown }): Promise<FhevmContractWrapper> {
  return (async () => {
    const client = developmentPublicClient(
      connection.provider as never,
      await developmentChain(connection.provider as never),
    );
    ...
    return repository.fhevmExecutor;
  })();
}
...
void test('the event vocabulary is exactly the executor ABI minus its proxy events', async () => {
  ...
  try {
    const executor = await executorOf(connection);
...
void test('parseCoprocessorEvents decodes executor logs in the viem and the ethers shape, and nothing else', async () => {
  ...
  try {
    ...
    assert.deepEqual(parseCoprocessorEvents(executor, null), []);
    assert.deepEqual(parseCoprocessorEvents(executor, undefined), []);
    assert.deepEqual(parseCoprocessorEvents(executor, [foreign]), []);
    const events = parseCoprocessorEvents(executor, [viemLog, ethersLog]);
...
void test('a live trivialEncrypt on the deployed executor yields one TrivialEncrypt event', async () => {
  ...
  try {
    ...
    const events = parseCoprocessorEvents(executor, receipt.logs);
