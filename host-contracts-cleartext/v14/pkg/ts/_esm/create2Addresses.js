 ...
 * ```
 */
export async function precomputeCreate2Addresses(parameters) {
    ...
    const proxyInitCode = (implementation, initData) => concatHex(erc1967ProxyTemplate.bytecode, ethUtils.encodeAbiParameters({ types: ['address', 'bytes'], values: [implementation, initData] }));
    ...
    const withAcl = (template) => patchTemplateBytecode({
        template,
        field: 'bytecode',
    ...
    const aclOwnerAddress = predict(CREATE2_ROLES.aclOwner, concatHex(aclOwnerTemplate.bytecode, ethUtils.encodeAbiParameters({ types: ['address', 'address'], values: [deployer, aclAddress] })));
    return {
        fhevmAddresses: {
            aclAddress,
