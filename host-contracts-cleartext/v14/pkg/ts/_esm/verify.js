 ...
 */
export async function snapshotStack(parameters) {
...
async function checkOwnership(report, ethProvider, deployed, expected) {
    ...
    report.expectAddress('ownership.ACLOwner.pendingOwner', await read(aclOwner, aclOwnerAbi, 'pendingOwner'), ZERO_ADDRESS);
...
async function checkBootstrap(report, ethProvider, deployed, expected) {
    ...
    if (expected.coprocessorSigners !== undefined) {
        const got = (await read(f.inputVerifierAddress, inputVerifierAbi, 'getCoprocessorSigners'));
    ...
    if (expected.coprocessorThreshold !== undefined) {
        report.expectEqual('bootstrap.coprocessorThreshold', await read(f.inputVerifierAddress, inputVerifierAbi, 'getThreshold'), expected.coprocessorThreshold);
    }
    if (expected.kmsSigners !== undefined) {
        const got = (await read(f.protocolConfigAddress, protocolConfigAbi, 'getKmsSigners'));
    ...
    if (expected.kmsContextId !== undefined) {
        report.expectEqual('bootstrap.kmsContextId', await read(f.protocolConfigAddress, protocolConfigAbi, 'getCurrentKmsContextId'), expected.kmsContextId);
    }
    ...
    if (thresholds !== undefined) {
        ...
        for (const [key, functionName] of getters) {
            report.expectEqual(`bootstrap.kmsThresholds.${key}`, await read(f.protocolConfigAddress, protocolConfigAbi, functionName), thresholds[key]);
        }
    }
}
////////////////////////////////////////////////////////////////////////////////
 ...
 * intended to touch.
 */
export const DEFAULT_MAY_CHANGE = [
