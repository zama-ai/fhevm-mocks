...
abstract contract HandlesSender is OAppSenderUpgradeable, ACLOwnable, BridgeEvents {
     ...
     * @dev    Pass 0 to clear the override and fall back to the default constant.
     */
    function setLzReceivePerPayloadByteGas(uint32 dstEid, uint64 lzReceivePerPayloadByteGas)
        external
        virtual
        onlyACLOwner
    {
    ...
    function _dispatch(
        ...
        bytes memory finalOptions = _buildOptions(
            dstEid, uint64(handleList.length), uint64(payload.length), lzComposeGas
        );
     ...
     * @dev Reverts with {ZeroLzComposeGas} if `lzComposeGas` is 0.
     */
    function _buildOptions(uint32 dstEid, uint64 nHandles, uint64 payloadLen, uint64 lzComposeGas)
        internal
        view
        virtual
        returns (bytes memory)
    {
        ...
        uint64 lzReceiveGas = _effectiveLzReceiveBaseGas(dstEid) + nHandles * _effectiveLzReceivePerHandleGas(dstEid)
            + payloadLen * _effectiveLzReceivePerPayloadByteGas(dstEid);
