...
abstract contract HandlesReceiver is OAppReceiverUpgradeable, ILayerZeroComposer, ACLOwnable, BridgeEvents {
    ...
    function _handleInbound(uint32 srcEid, bytes32 guid, bytes calldata message) internal virtual {
        ...
        (bytes32 srcApp, bytes32 dstApp, bytes memory payload, bytes32[] memory srcHandleList) =
            abi.decode(message, (bytes32, bytes32, bytes, bytes32[]));
        ...
        endpoint.sendCompose(
            address(this), guid, 0, abi.encode(srcEid, srcApp, dstApp, payload, srcHandleList, dstHandleList)
        );
    }
     ...
     */
    function _deriveAndEmit(bytes32 dstApp, bytes32[] memory srcHandleList, bytes32 guid)
        internal
        virtual
        returns (bytes32[] memory dstHandleList)
    {
