...
contract ProtocolConfig is IProtocolConfig, UUPSUpgradeableEmptyProxy, ACLOwnable {
    ...
    struct ProtocolConfigStorage {
        ...
        mapping(uint256 epochId => mapping(bytes32 dataHash => uint256 confirmations))
            epochActivationConfirmationCountForDigest;
    ...
    function initializeFromEmptyProxy(
        ...
        uint256 newContextId = _storeAndActivateKmsContextAndEpoch(
            KMS_CONTEXT_COUNTER_BASE + 1, EPOCH_COUNTER_BASE + 1, initialKmsNodeParams, initialThresholds
        );
        ...
        emit NewKmsContext(
            newContextId, KMS_CONTEXT_COUNTER_BASE, initialKmsNodeParams, initialThresholds, softwareVersion, pcrValues
        );
    }
    ...
    function initializeFromCanonical(
        ...
        _storeAndActivateKmsContextAndEpoch(
            canonicalContextId, canonicalEpochId, canonicalKmsNodeParams, canonicalThresholds
        );
    }
    ...
    function defineNewKmsContextAndEpoch(
        ...
        uint256 previousQuorum =
            $.kmsNodesForContext[previousContextId].length - $.mpcThresholdForContext[previousContextId];
    ...
    function defineNewEpochForCurrentKmsContext() external virtual onlyACLOwner {
        ...
        emit NewKmsEpoch(
            latestActiveKmsContextId, epochId, latestActiveKmsContextId, $.latestActiveEpochId, block.number - 1
        );
    }
    ...
    function confirmEpochActivation(uint256 epochId, EpochKeyResult[] calldata keys, EpochCrsResult[] calldata crsList)
        external
        virtual
    {
        ProtocolConfigStorage storage $ = _getProtocolConfigStorage();
        ...
        bytes32 dataHash;
        {
            ...
            for (uint256 i = 0; i < keys.length; i++) {
                ...
                bytes32 digest = _hashKeygenVerification(keys[i].prepKeygenId, keys[i].keyId, keyDigestsHash, extraData);
            ...
            for (uint256 i = 0; i < crsList.length; i++) {
                bytes32 digest =
                    _hashCrsgenVerification(crsList[i].crsId, crsList[i].maxBitLength, crsList[i].crsDigest, extraData);
    ...
    function destroyKmsEpoch(uint256 epochId) external virtual onlyACLOwner {
        ...
        bool isEpochPendingUnderActiveContext =
    ...
    function _storeNextKmsContext(KmsNodeParams[] memory kmsNodeParams, KmsThresholds calldata thresholds)
        internal
        virtual
    ...
    function _hasContextCreationQuorum(uint256 contextId) internal view virtual returns (bool) {
        ProtocolConfigStorage storage $ = _getProtocolConfigStorage();
        return $.contextCreationNewTxSenderConfirmationCount[contextId] == $.kmsNodesForContext[contextId].length
            && $.contextCreationPreviousTxSenderConfirmationCount[contextId]
                >= $.contextCreationPreviousTxSenderThreshold[contextId];
    }

    function _requireExpectedSigner(address expectedSigner, bytes32 digest, bytes calldata signature)
        internal
        view
        virtual
    {
    ...
    function _hashKeygenVerification(
        ...
        return _hashTypedData(
            keccak256(abi.encode(EIP712_KEYGEN_TYPE_HASH, prepKeygenId, keyId, keyDigestsHash, keccak256(extraData)))
        );
    }
