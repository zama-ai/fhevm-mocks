 ...
 * `fhevm-config-0.14.0/addresses.sol` and `forge build` has been executed with
 * that directory remapped, so that all implementation contracts carry the
 * correct baked-in addresses.
 *
 * Usage (from the package root):
 *
 *   FOUNDRY_REMAPPINGS=fhevm-config-0.14.0/=internal/.deploy-config/ \
 ...
 *    `updateV13ToV14` requires the ACL owner to already be an ACLOwner rather
 *    than an EOA.
 *
 * ---------------------------------------------------------------------------
...
contract FhevmDeployScript is Script {
    ...
    /// @dev Seeds the initial KMS context; v14 also names the KMS build and accepts no enclave PCRs.
    function _protocolConfigInit() private pure returns (bytes memory) {
        return abi.encodeCall(
            ProtocolConfig.initializeFromEmptyProxy,
            (_defaultKmsNodes(), _defaultKmsThresholds(), LocalHostBootstrap.KMS_SOFTWARE_VERSION, new PcrValues[](0))
        );
    }

    /// @dev The four default KMS nodes, assembled from LocalHostBootstrap's parallel arrays; no CA cert, no MPC peer.
    function _defaultKmsNodes() private pure returns (KmsNodeParams[] memory nodes) {
        ...
        nodes = new KmsNodeParams[](LocalHostBootstrap.KMS_NODE_COUNT);
