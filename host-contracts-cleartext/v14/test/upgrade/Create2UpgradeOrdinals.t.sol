 ...
 * @notice Pins the index alignment of the independent 7-create / 7-op table used only by the
 *         v13 -> v14 upgrade, the way its sibling `Create2Ordinals.t.sol` pins the fresh deploy's lists.
 *
...
contract Create2UpgradeOrdinalsTest is Test, FhevmUpgradeBase {
    ...
    /// One implementation create per op, and nothing else: v14 adds no proxy to the live stack.
    function test_upgradeArtifactsResolveAndCreateCountIsSeven() public view {
        string[] memory roles = _upgradeProxyRoles();
        for (uint256 i; i < roles.length; i++) {
            assertGt(vm.getCode(_upgradeImplArtifact(i)).length, 0);
        }
        assertEq(_upgradeCreateTable().length, 7, "seven implementations, no empty implementation, no proxy");
    }

    /// The untouched set and the op set partition the nine live proxies.
    function test_untouchedAndUpgradedRolesPartitionTheStack() public pure {
        string[] memory untouched = _untouchedProxyRoles();
        assertEq(untouched.length, 2, "untouched count");
        assertEq(untouched[0], "INPUT_VERIFIER_ADDRESS");
        assertEq(untouched[1], "CLEARTEXT_DB_ADDRESS");
        assertEq(
            untouched.length + _upgradeProxyRoles().length,
            _allProxyRoles().length,
            "every live proxy is one or the other"
        );
    }
