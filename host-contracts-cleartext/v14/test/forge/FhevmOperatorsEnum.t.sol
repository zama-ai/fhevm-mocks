...
import {Operators} from "../../pkg/src/cleartext/shared/FhevmOperatorsEnum.sol";

/// `cleartext/shared/FhevmOperatorsEnum.sol` re-declares the vendored executor's operator list so the
...
contract FhevmOperatorsTest is Test {
    ...
    function test_everyOperatorHasTheSamePosition() public pure {
        ...
        assertEq(uint8(Operators.fheMulDiv), uint8(Vendored.Operators.fheMulDiv), "fheMulDiv");
    }
    ...
    function test_neitherSideHasGrownAMember() public pure {
        ...
        assertEq(uint8(type(Operators).max), 30, "and the count is the one this file was written against");
    }
}
