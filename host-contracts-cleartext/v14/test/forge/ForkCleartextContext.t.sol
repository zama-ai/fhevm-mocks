...
contract ForkCleartextContextTest is Test {
    ...
    function test_theForkStartsWithSignersWeCannotSignAs() public onlyForked {
        assertFalse(
            LibForgeFhevmStack.hasCleartextCoprocessorContext(inputVerifier),
    ...
    function test_theContextBecomesTheCleartextOne() public onlyForked {
        LibForgeFhevmStack.defineCleartextCoprocessorContext(inputVerifier, acl);
        ...
        assertTrue(LibForgeFhevmStack.hasCleartextCoprocessorContext(inputVerifier), "and the helper agrees");
    }
    ...
    function test_withoutTheAclOwnerTheContextCannotBeDefined() public onlyForked {
        address[] memory signers = LibForgeFhevmStack.cleartextCoprocessorSigners();
        uint256 threshold = LibForgeFhevmStack.cleartextCoprocessorThreshold();

        vm.prank(makeAddr("not the owner"));
        vm.expectRevert();
        IInputVerifier(inputVerifier).defineNewContext(signers, threshold);

        assertFalse(LibForgeFhevmStack.hasCleartextCoprocessorContext(inputVerifier), "unchanged");
    }
    ...
    function test_theOwnerIsReadFromTheForkedAcl() public onlyForked {
        address owner = LibForgeFhevmStack.aclOwner(acl);
        assertTrue(owner != address(0), "the ACL names an owner");

        vm.prank(owner);
        IInputVerifier(inputVerifier)
            .defineNewContext(
                LibForgeFhevmStack.cleartextCoprocessorSigners(), LibForgeFhevmStack.cleartextCoprocessorThreshold()
            );
        assertTrue(LibForgeFhevmStack.hasCleartextCoprocessorContext(inputVerifier));
    }
    ...
    function test_theForkStartsWithKmsSignersWeCannotSignAs() public onlyForked {
        assertFalse(LibForgeFhevmStack.hasCleartextKmsContext(kmsVerifier));
    ...
    function test_theKmsContextBecomesTheCleartextOne() public onlyForked {
        LibForgeFhevmStack.defineCleartextKmsContext(protocolConfig, acl);
        ...
        assertTrue(LibForgeFhevmStack.hasCleartextKmsContext(kmsVerifier), "and the helper agrees");
    }
    ...
    function test_theKmsNodesAreTheBootstrapOnes() public onlyForked {
        IProtocolConfig.KmsNodeParams[] memory nodes = LibForgeFhevmStack.cleartextKmsNodes();
    ...
    /// The helper drives the proposal through creation and activation, so the new context is the current one.
    ...
    function test_withoutTheAclOwnerTheKmsContextCannotBeDefined() public onlyForked {
        ...
        IProtocolConfig(protocolConfig)
            .defineNewKmsContextAndEpoch(
                ...
                LibForgeFhevmStack.cleartextKmsThresholds(),
                LocalHostBootstrap.KMS_SOFTWARE_VERSION,
                new IProtocolConfig.PcrValues[](0)
            );

        assertFalse(LibForgeFhevmStack.hasCleartextKmsContext(kmsVerifier), "unchanged");
    }
    ...
    function test_bothContextsAreSwappedTogether() public onlyForked {
        LibForgeFhevmStack.defineCleartextContexts(inputVerifier, protocolConfig, acl);

        assertTrue(LibForgeFhevmStack.hasCleartextCoprocessorContext(inputVerifier), "inputs");
        assertTrue(LibForgeFhevmStack.hasCleartextKmsContext(kmsVerifier), "decryptions");
    }
}
