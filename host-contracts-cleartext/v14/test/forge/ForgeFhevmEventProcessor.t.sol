...
contract ForgeFhevmEventProcessorTest is Test, ForgeFhevmDeploy {
    ...
    /// v14's `fheMulDiv`: the product is widened, so the intermediate overflows uint8 and the result does not.
    function test_mulDiv() public {
        bytes32 factor1 = executor.trivialEncrypt(200, FheType.Uint8);
        bytes32 factor2 = executor.trivialEncrypt(200, FheType.Uint8);

        _assertAgrees(executor.fheMulDiv(factor1, factor2, bytes32(uint256(200)), 0x01), 200);
    }

    function test_mulDivScalar() public {
        bytes32 factor1 = executor.trivialEncrypt(50, FheType.Uint8);

        _assertAgrees(executor.fheMulDiv(factor1, bytes32(uint256(3)), bytes32(uint256(5)), 0x03), 30);
    }
