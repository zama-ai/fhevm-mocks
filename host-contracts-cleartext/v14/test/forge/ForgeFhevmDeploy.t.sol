...
import {ICleartextACL} from "../../pkg/forge/src/ForgeFhevmDeploy.sol";
import {ICleartextArithmetic} from "../../pkg/forge/src/ForgeFhevmDeploy.sol";
...
/**
 * The Foundry half of the suite: `ForgeFhevmDeploy` is the one artifact a TS test cannot exercise, because it
 ...
 */
contract FhevmDeployTest is Test, ForgeFhevmDeploy {
    ...
    function test_theCleartextGenerationIsThirteen() public pure {
        assertEq(IForgeMarker(ACL_ADDRESS).CLEARTEXT_PROTOCOL_VERSION(), 14, "acl");
    }
 ...
 * produces exactly one stack — see the note in ForgeFhevmDeploy about why a configurable one would be a
 * liability rather than a feature.
 */

/// The determinism guard: every address derives from the deployer's nonce, so a dirty one must abort.
contract FhevmDeployGuardTest is Test, ForgeFhevmDeploy {
    function test_refusesToDeployFromADirtyNonce() public {
        vm.setNonce(DEPLOYER_ADDRESS, uint64(DEPLOYER_START_NONCE + 3));
        vm.expectRevert(
            bytes("ForgeFhevmDeploy: deployer nonce must be DEPLOYER_START_NONCE; every address derives from it")
        );
