...
//
// The sibling `upgrade/create2.test.ts` starts from a v13 stack and upgrades it; this one has no previous
...
void test(
  'create2-deploy: fresh anvil, a v14 stack through the CREATE2 coordinator',
  { skip: blockedReason() },
  async (t) => {
    if (await portIsOpen()) {
    ...
    try {
      ...
      await t.test('the coordinator deploys and verifies the whole stack in one run', async () => {
        announce(
          2,
          STEPS,
          'deploy v14 with deploy-testnet.ts --stage all (3 forge builds, 22 creates, steps A-F, verify)',
        );
        ...
        if (!ok) {
          ...
          assert.fail(`v14 create2 deploy failed${detail}:\n${output.slice(-4000)}`);
        }
      ...
      await t.test('every contract reports its v14 version at its sealed address', async (st) => {
