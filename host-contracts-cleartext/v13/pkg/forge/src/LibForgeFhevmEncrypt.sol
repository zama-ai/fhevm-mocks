...
import {LibInputVerifier} from "./shared/LibInputVerifier.sol";
 ...
 */
/**
 * @notice The two addresses an encryption is computed against.
 *
 * @dev BOTH ARE PART OF THE ANSWER, not just of the proof. The ACL address is mixed into the blob hash
 *      and into every handle, so encrypting against the wrong one yields handles the chain will never
 *      recognise — silently, since nothing checks them until the dApp call fails. The input verifier is
 *      what says which signers the proof must carry.
 */
struct EncryptStack {
    address inputVerifier;
    address acl;
    /// @dev Whether `inputVerifier` is a cleartext one that may be ASKED for the proof directly, rather
    ///      than the proof being rebuilt from the coprocessor signer keys this package holds. Decided by
    ///      the caller (forge-fhevm-std's `fhevm.useCleartextVerifier()`): this library does not read the
    ///      environment or probe the chain — it takes the decision as data.
    bool cleartextVerifier;
}

library LibForgeFhevmEncrypt {
     ...
     *
     *      THIS LIBRARY DOES NOT TOUCH GAS METERING. It stands in for the SDK, so its cost must not
     *      land in a test's gas figure — but the pausing belongs to the caller: `ForgeVmBase.unmetered`
     *      wraps each public entry point and counts depth, and a library that resumed on its own would
     *      re-enable metering in the middle of that scope.
     */
    ...
    {
        return encrypt(typeIds, values, contractAddress, userAddress, localStack());
    }

    /// @notice Same, against a named stack — a forked one, whose ACL and input verifier are its own.
    function encrypt(
        uint8[] memory typeIds,
        uint256[] memory values,
        address contractAddress,
        address userAddress,
        EncryptStack memory stack
    ) internal view returns (bytes32[] memory handles, bytes memory inputProof) {
        (handles, inputProof) = _encrypt(typeIds, values, contractAddress, userAddress, stack);
    }

    /// @notice The locally deployed cleartext stack, for a caller that names none.
    function localStack() internal pure returns (EncryptStack memory) {
        return EncryptStack({inputVerifier: INPUT_VERIFIER_ADDRESS, acl: ACL_ADDRESS, cleartextVerifier: true});
    }
    ...
    function _encrypt(
        ...
        address userAddress,
        EncryptStack memory stack
    ) private view returns (bytes32[] memory handles, bytes memory inputProof) {
        if (typeIds.length != values.length) {
            revert InputLengthMismatch(typeIds.length, values.length);
        }
        ...
        bytes32 blobHash = _blobHash(values, contractAddress, userAddress, stack.acl);

        bytes memory cleartextExtraData = _cleartextExtraData(values);

        handles = new bytes32[](n);
        for (uint256 i = 0; i < n; i++) {
            handles[i] = _inputHandle(blobHash, i, typeIds[i], stack.acl);
        }
        ...
        bytes memory signatures = LibForgeFhevmSigners.packSignatures(
            LibForgeFhevmSigners.randomCoprocessorSignatures(digest, signers, threshold)
        );
     ...
     *      (`eip712Domain`, the typehash, the signer set). `LibInputVerifier` does that, and the CALLER
     *      picks between them through `EncryptStack.cleartextVerifier` (forge-fhevm-std decides it from
     *      `IS_CLEARTEXT` and the run's flags), so neither path needs configuring here.
     *
    ...
    function _inputProof(
        ...
        address inputVerifier,
        bool cleartextVerifier
    ) private view returns (bytes32 digest, address[] memory signers, uint256 threshold) {
        if (cleartextVerifier) {
            return ICleartextInputVerifier(inputVerifier).inputProof(handles, userAddress, contractAddress, extraData);
        }
        return LibInputVerifier.inputProof(inputVerifier, handles, userAddress, contractAddress, extraData);
    }
    ...
    function encrypt(bytes memory abiEncodedTypeValuePairs, address contractAddress, address userAddress)
        ...
        returns (bytes32[] memory handles, bytes memory inputProof)
    {
        return encrypt(abiEncodedTypeValuePairs, contractAddress, userAddress, localStack());
    }

    /// @notice Same, against a named stack.
    function encrypt(
        bytes memory abiEncodedTypeValuePairs,
        address contractAddress,
        address userAddress,
        EncryptStack memory stack
    ) internal view returns (bytes32[] memory handles, bytes memory inputProof) {
     ...
     */
    function _blobHash(uint256[] memory values, address contractAddress, address userAddress, address acl)
        private
        view
        returns (bytes32)
    {
        bytes memory metaData = abi.encodePacked(contractAddress, userAddress, acl, block.chainid);
     ...
     */
    function _inputHandle(bytes32 blobHash, uint256 index, uint8 typeId, address acl) private view returns (bytes32) {
        bytes21 hash21 =
            bytes21(keccak256(abi.encodePacked(HANDLE_HASH_DOMAIN, blobHash, uint8(index), acl, block.chainid)));
