...
contract FhevmCleartextEncryptTest is Test, FhevmCleartextDeploy {
    ...
    function test_pairEncodedOverloadIsAlsoAccepted() public {
        (bytes32[] memory handles, bytes memory proof) = FhevmCleartextEncrypt.encrypt(
    ...
    function test_theSameValueTwiceYieldsDifferentHandles() public {
        (uint8[] memory typeIds, uint256[] memory values) = _one(uint8(FheType.Uint32), 4242);
        (bytes32[] memory first,) = FhevmCleartextEncrypt.encrypt(typeIds, values, address(this), alice);
        (bytes32[] memory second,) = FhevmCleartextEncrypt.encrypt(typeIds, values, address(this), alice);
    ...
    function test_aProofBoundToAnotherContractIsRejected() public {
        (uint8[] memory typeIds, uint256[] memory values) = _one(uint8(FheType.Uint32), 4242);
        (bytes32[] memory handles, bytes memory proof) =
            FhevmCleartextEncrypt.encrypt(typeIds, values, address(0xBEEF), alice);
    ...
    function test_mismatchedArrayLengthsRevert() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(FhevmCleartextEncrypt.InputLengthMismatch.selector, 2, 1));
        this.callEncrypt(typeIds, values);
    }

    function test_anEmptyBundleReverts() public {
        vm.expectRevert(abi.encodeWithSelector(FhevmCleartextEncrypt.TooManyInputs.selector, 0));
    ...
    function test_moreThan254InputsReverts() public {
        vm.expectRevert(abi.encodeWithSelector(FhevmCleartextEncrypt.TooManyInputs.selector, 255));
        this.callEncrypt(new uint8[](255), new uint256[](255));
    }

    function test_aPairBufferThatIsNotAMultipleOf64Reverts() public {
        vm.expectRevert(abi.encodeWithSelector(FhevmCleartextEncrypt.MalformedTypeValuePairs.selector, 32));
    ...
    function test_anOutOfRangeTypeIdReverts() public {
        vm.expectRevert(abi.encodeWithSelector(FhevmCleartextEncrypt.InvalidFheTypeId.selector, 256));
    ...
    function _proof() private returns (bytes32 handle, bytes memory proof) {
        ...
        (handles, proof) = FhevmCleartextEncrypt.encrypt(typeIds, values, address(this), alice);
        handle = handles[0];
    }
    ...
    function callEncryptPairs(bytes calldata pairs) external {
        FhevmCleartextEncrypt.encrypt(pairs, address(this), alice);
    }
