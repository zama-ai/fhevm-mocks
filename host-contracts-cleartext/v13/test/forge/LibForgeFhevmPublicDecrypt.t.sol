...
import {LibForgeFhevmPublicDecrypt} from "../../pkg/forge/src/LibForgeFhevmPublicDecrypt.sol";
import {FheType} from "../../pkg/forge/src/shared/LibFheType.sol";
import {Bits} from "../utils/Bits.sol";

/**
 * `LibForgeFhevmPublicDecrypt` produces a result the real `KMSVerifier` accepts, and the values it
 * carries are the ones the stack holds.
 *
 * Same shape as `LibForgeFhevmEncrypt.t.sol`, for the same reason: the library is only useful if the
 ...
 */
contract LibForgeFhevmPublicDecryptTest is Test, ForgeFhevmDeploy {
    ...
    function test_aPubliclyDecryptableHandleYieldsAVerifiableProof() public {
        ...
        (bytes memory values, bytes memory proof) = LibForgeFhevmPublicDecrypt.decryptPublicWithProof(handles);
    ...
    function test_theProofCarriesOneSignaturePerRegisteredSigner() public {
        bytes32[] memory handles = _decryptable(4242);
        (, bytes memory proof) = LibForgeFhevmPublicDecrypt.decryptPublicWithProof(handles);
