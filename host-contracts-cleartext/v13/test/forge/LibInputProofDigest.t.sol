...
import {INPUT_VERIFIER_ADDRESS, ACL_ADDRESS} from "../../pkg/forge/src/FhevmCleartextDeploy.sol";
import {ICleartextInputVerifier} from "../../pkg/forge/src/_internal/interfaces/ICleartextInputVerifier.sol";
import {LibInputProofDigest} from "../../pkg/forge/src/shared/LibInputProofDigest.sol";
...
contract LibInputProofDigestTest is Test, FhevmCleartextDeploy {
    ...
    function _assertAgrees(bytes32[] memory handles, bytes memory extraData) private view {
        ...
        (bytes32 d2, address[] memory s2, uint256 t2) =
            LibInputProofDigest.inputProof(INPUT_VERIFIER_ADDRESS, handles, alice, dapp, extraData);
    ...
    function test_theDigestTracksItsInputs() public view {
        ...
        (bytes32 a,,) = LibInputProofDigest.inputProof(INPUT_VERIFIER_ADDRESS, h, alice, dapp, hex"00");
        (bytes32 b,,) = LibInputProofDigest.inputProof(INPUT_VERIFIER_ADDRESS, h, alice, dapp, hex"01");
        (bytes32 c,,) = LibInputProofDigest.inputProof(INPUT_VERIFIER_ADDRESS, h, dapp, dapp, hex"00");
        (bytes32 d,,) = LibInputProofDigest.inputProof(INPUT_VERIFIER_ADDRESS, _handles(3), alice, dapp, hex"00");
    ...
    function test_theCleartextProbeDiscriminates() public view {
        assertTrue(LibInputProofDigest.isCleartextVerifier(INPUT_VERIFIER_ADDRESS), "the cleartext verifier");
        assertTrue(LibInputProofDigest.isCleartextVerifier(ACL_ADDRESS), "the rest of the cleartext stack");
        ...
        assertFalse(LibInputProofDigest.isCleartextVerifier(address(this)), "code, but no such function");
        assertFalse(LibInputProofDigest.isCleartextVerifier(address(0xDEAD)), "no code at all");
    }
}
