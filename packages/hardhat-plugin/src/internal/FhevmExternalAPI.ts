...
export class FhevmExternalAPI implements HardhatFhevmRuntimeEnvironment {
  ...
  public get isMock(): boolean {
    return this.fhevmEnv.mockProvider.isMock;
  }
  ...
  public parseCoprocessorEvents(logs: (EthersT.EventLog | EthersT.Log)[] | null | undefined): CoprocessorEvent[] {
    ...
    for (const log of logs) {
      let parsed: EthersT.LogDescription | null = null;
      try {
  ...
  public async assertCoprocessorInitialized(contract: AddressLike, contractName?: string): Promise<void> {
    ...
    const prefix = contractName === undefined ? `Contract at ${contractAddress}` : `Contract ${contractName} at ${contractAddress}`;
  ...
  public async publicDecrypt(handles: (string | Uint8Array)[]): Promise<PublicDecryptResults> {
    ...
    */
    handles.forEach((h) => assertHandleIsInitialized(__handleKey(h)));
