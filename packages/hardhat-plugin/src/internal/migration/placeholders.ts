...
/**
 * OWN — the v13 cleartext stack adds `ProtocolConfig`, `KMSGeneration`, `CleartextArithmetic`,
 * `CleartextDB` and `PauserSet`. Widened here already so `revertedWithCustomErrorArgs` can name them;
 * ABIs come from `@fhevm/host-contracts-cleartext`'s `./abi/*.json` export.
 */
...
/** OWN — HCU accounting is the plugin's own feature; `operatorsPrices.json` stays with it. */
