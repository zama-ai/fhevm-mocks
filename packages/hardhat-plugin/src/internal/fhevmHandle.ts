...
export function parseFhevmHandle(handleBytes32: string): FhevmHandleInfo {
  if (typeof handleBytes32 !== "string" || !EthersT.isHexString(handleBytes32, 32)) {
    throw new HardhatFhevmError(
      `Invalid FHE handle '${String(handleBytes32)}': expected a 32-byte 0x-prefixed hex string.`,
    );
  }
  ...
  const fhevmType = typeByte as FhevmType;
