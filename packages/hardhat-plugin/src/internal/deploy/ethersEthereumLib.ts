// Vendored VERBATIM from the `@fhevm/host-contracts-cleartext` reference adapter:
//   <host-contracts-cleartext>/test/ts/utils/ethersEthereumLib.ts
//
// Keep it byte-identical to upstream. Re-sync by copying that file over this one and re-adding this
// header — nothing else. It is listed in .prettierignore precisely so this repo's formatter cannot
// introduce a permanent diff against upstream, so do not reformat or hand-edit it either. Anything
// this project needs on top belongs in setup.ts, not here.
