// Ported from `@fhevm/mock-utils` (`fhevm/coprocessor/HCUByOperator.ts`), which the plugin no longer
// depends on. The pricing data itself is unchanged; only the imports were rehomed.
import { HardhatFhevmError } from "../../error";
import type { FheTypeName } from "./fheTypeName";
import type { CoprocessorOperatorEventName } from "./events";
...
export function getHCU(opName: HCUOperatorName, type: FheTypeName, opts?: { scalar: boolean }): number {
  ...
  if (!m[type]) {
    throw new HardhatFhevmError(`Type ${type} not supported by HCU operator '${opName}'`);
  }
