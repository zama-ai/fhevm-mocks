...
import { HCUByOperator } from "./HCUByOperator";
import { type FheTypeName, getFheTypeName } from "./fheTypeName";
  ...
  for (const event of FHELogs) {
    ...
    switch (event.name) {
      case "TrivialEncrypt": {
        ...
        const toFheTypeName: FheTypeName = getFheTypeName(Number(toFheType));
      ...
      case "FheRand": {
        ...
        const randTypeFheTypeName: FheTypeName = getFheTypeName(Number(randTypeUint8));
