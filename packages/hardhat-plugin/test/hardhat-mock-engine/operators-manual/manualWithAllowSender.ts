...
describe("FHEVM manual operations using userDecrypt", function () {
  ...
  it("Select ebytes64", async function () {
    ...
    await tx.wait();
    const res = await hre.fhevm.userDecryptEbytes(
      FhevmType.ebytes64,
      await contract.resEbytes64(),
      contractAddress,
      signers.alice,
    );
    expect(res).to.equal(
    ...
    await tx2.wait();
    const res2 = await hre.fhevm.userDecryptEbytes(
      FhevmType.ebytes64,
      await contract.resEbytes64(),
      contractAddress,
      signers.alice,
  ...
  it("eq ebytes256,ebytes256 true", async function () {
    ...
    const decRes = await hre.fhevm.userDecryptEbool(res, contractAddress, signers.alice);
